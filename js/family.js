/* ==========================================================================
   FAMILY PORTAL, EMAIL OTP (yaikobtesgera0@gmail.com), & CLOUDINARY GALLERY
   ========================================================================== */
...
/* --------------------------------------------------------------------------
   2. FAMILY AUTHENTICATION, EMAIL OTP & CLOUDINARY GALLERY WITH SWIPE LIGHTBOX
   -------------------------------------------------------------------------- */
function initFamilyPortal() {
    ...
    if (lightboxImg) {
        // Touch events for mobile phones & touch screens
    ...
    function uploadToCloudinary(e) {
        ...
        let storedPhotos = JSON.parse(localStorage.getItem("cloudinaryPhotos") || "[]");
        ...
        files.forEach(file => {
            ...
            .then(data => {
                if (data.secure_url) {
                    if (!storedPhotos.includes(data.secure_url)) {
                        storedPhotos.unshift(data.secure_url);
                        
                        try {
                            localStorage.setItem("cloudinaryPhotos", JSON.stringify(storedPhotos));
                        } catch (err) {
                            localStorage.clear();
                            localStorage.setItem("cloudinaryPhotos", JSON.stringify([data.secure_url]));
                        }
    ...
    function loadSavedPhotos() {
        const storedPhotos = JSON.parse(localStorage.getItem("cloudinaryPhotos") || "[]");
        renderGallery(storedPhotos);
    }
    ...
    function renderGallery(photos) {
        ...
        photos.forEach((src, index) => {
            ...
            // Open full-screen lightbox at the clicked index
