...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", (e) => {
            ...
            // Trim whitespace to prevent typo issues
...
// Dynamic Profile Photo Uploaders
...
if (husbandInput) {
    husbandInput.addEventListener('change', (e) => {
        ...
        if (file) {
            document.getElementById('husband-photo-img').src = URL.createObjectURL(file);
        }
    });
}
...
if (wifeInput) {
    wifeInput.addEventListener('change', (e) => {
        const file = e.target.files[0];
        if (file) {
            document.getElementById('wife-photo-img').src = URL.createObjectURL(file);
        }
    });
}